...
class DeliveryOrderDao {
  static getLastedDeliveryOrderRecords() async {
    ...
    if (res != null && res.result) {
      print(
          "get lasted delivery order records success : " + res.data.toString());
      return DataResult(res.data, res.result, res.code);
    }
    if (res != null && !res.result) {
      print(
          "get lasted delivery order records failed : " + res.data.toString());
  ...
  static getHistoryBill(dateBegin, dateEnd, skipCount) async {
    var res;
    res = await HttpManager.netFetch(
        Address.getHistoryBill() +
            "?StartGenerateDate=${dateBegin}&EndGenerateDate=${dateEnd}&MaxResultCount=${Config.PAGE_SIZE}&SkipCount=${skipCount}",
        null,
        null,
        null);
    if (res.code == Config.ERROR_CODE401 || res.code == Config.ERROR_CODE403) {
    ...
    if (res != null && res.result) {
      print("get history delivery order records success : " +
          res.data.toString());
