...
class HttpManagerForGit {
  ...
  ///[ header] 外加头
  ///[ option] 配置
  static netFetchForGit(url, params, Map<String, String> header, Options option, {noTip = false}) async {
    ...
    try {
      if (option.contentType != null && option.contentType.primaryType == "text") {
        ...
        if (response.statusCode == 201 && responseJson["token"] != null) {
          ...
          await LocalStorage.save(Config.TOKEN_KEY_GIT, optionParams["authorizationCode"]);
        }
      }
  ...
  static clearAuthorization() {
    ...
    LocalStorage.remove(Config.TOKEN_KEY_GIT);
  }

  ///获取授权token
  static getAuthorization() async {
    String token = await LocalStorage.get(Config.TOKEN_KEY_GIT);
    if (token == null) {
      String basic = await LocalStorage.get(Config.USER_BASIC_CODE_GIT);
      if (basic == null) {
        //提示输入账号密码
      } else {
